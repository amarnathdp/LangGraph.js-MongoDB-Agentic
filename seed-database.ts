...
const embeddings = new GoogleGenerativeAIEmbeddings({
  apiKey: process.env.GEMINI_API_KEY,
  model: "gemini-embedding-001", // 3072 dimensions  // 3072 ?? Google recommends 3072, 1536, or 768 dimensions for quality and efficiency.
...
async function generateSyntheticData(): Promise<Employee[]> {
  ...
  Generate 10 fictional employee records. Each record should include 
  the following fields: employee_id, first_name, last_name, date_of_birth, address, contact_details, job_details, work_location, reporting_manager, skills, performance_reviews, benefits, emergency_contact, notes. 
  Ensure variety in the data and realistic values.

  ${parser.getFormatInstructions()}`; // This is for additional instructions from ZOD, check the doc..
